...
def menu():
    return [
        ...
        [Button.inline("📄 Xem Code", b"codes")],
...
async def cb(e):
    if e.data == b"acc":
        ...
        if not CODES_DB:
            txt = "📄 Chưa có code nào."
        else:
            txt = "📄 DANH SÁCH CODE HIỆN CÓ\n"
            for acc_id, code in CODES_DB.items():
                name = ACCS.get(int(acc_id), {}).get("name", "Unknown")
                txt += f"- {name}: `{code}`\n"
        await e.edit(txt, buttons=[[Button.inline("⬅️ Back", b"back")]])
...
async def grab_loop(acc):
    ...
    async def handler(ev):
        ...
        try:
            ...
            acc["status"] = "ERROR"
            print(f"❌ Lỗi grab_loop {acc['name']}: {e}")
